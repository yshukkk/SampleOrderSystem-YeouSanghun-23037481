...
def test_register_assigns_given_id_and_starts_stock_at_zero(tmp_path):
    repository = make_repository(tmp_path)

    sample = repository.register(id=1, name="시료-A", avg_production_time=2.5, yield_rate=0.9)
...
def test_register_uses_the_caller_supplied_ids(tmp_path):
    repository = make_repository(tmp_path)

    first = repository.register(id=10, name="시료-A", avg_production_time=1.0, yield_rate=1.0)
    second = repository.register(id=20, name="시료-B", avg_production_time=2.0, yield_rate=0.8)

    assert first.id == 10
    assert second.id == 20


def test_register_with_duplicate_id_returns_none(tmp_path):
    repository = make_repository(tmp_path)
    repository.register(id=1, name="시료-A", avg_production_time=1.0, yield_rate=1.0)

    result = repository.register(id=1, name="시료-B", avg_production_time=2.0, yield_rate=0.5)

    assert result is None
    # The original record must not have been overwritten.
    original = repository.find(1)
    assert original.name == "시료-A"


def test_list_all_returns_registered_samples_in_order(tmp_path):
    repository = make_repository(tmp_path)
    repository.register(id=1, name="시료-A", avg_production_time=1.0, yield_rate=1.0)
    repository.register(id=2, name="시료-B", avg_production_time=2.0, yield_rate=0.8)
...
def test_find_success(tmp_path):
    repository = make_repository(tmp_path)
    registered = repository.register(id=1, name="시료-A", avg_production_time=1.0, yield_rate=1.0)
...
def test_search_by_name_matches_substring_case_insensitively(tmp_path):
    ...
    repository.register(id=2, name="wafer-B", avg_production_time=1.0, yield_rate=1.0)
    repository.register(id=3, name="Chip-C", avg_production_time=1.0, yield_rate=1.0)
...
def test_repository_persists_across_repository_instances(tmp_path):
    ...
    SampleRepository(JsonRepository(path)).register(
        id=1, name="시료-A", avg_production_time=1.0, yield_rate=1.0
    )
